...
public class ServerNetwork : UCNetwork
{
    ...
    [HideInInspector] public Dictionary<int, NetworkObject> networkObjects = new Dictionary<int, NetworkObject>();
    ...
    override protected void HandleMessage_StatusChanged(NetIncomingMessage aMsg)
    {
        ...
        if (status == NetConnectionStatus.Connected)
        {
            ...
            clientData.Add(data);
            //ryan
            //GetComponent<TagServer>().SetNetId(data.networkIdentifier);
        ...
        else if (status == NetConnectionStatus.Disconnected)
        {
            ...
            for (int i = 0; i < clientData.Count; i++)
            {
                if (clientData[i].networkIdentifier == aMsg.SenderConnection.RemoteUniqueIdentifier)
                {
                    ...
                    {
                        if (2 == networkObjects.Count && netObj.Value.it == false)//if one player they are it
                        {
                            netObj.Value.it = true;
                            CallRPC("YouIt", UCNetwork.MessageReceiver.AllClients, netObj.Value.networkId);
                        }
                    }
